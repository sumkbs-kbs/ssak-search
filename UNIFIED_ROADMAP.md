...
### 평가 인프라 전환: Self-hosted Runner + 베이스라인 재앵커링 (2026-08-25)

Azure 러너 2회 완주 비교로 봇월 가설 데이터 확정 → **Evaluation을
Self-hosted Runner(Mac, 거주 IP, label ssak-mac)로 영구 전환**:

| 지표 | Azure 러너 | Mac 거주 IP |
|---|---|---|
| NDCG@10 | 0.3236 | **0.3617** (+11.7%) |
| Pass Rate | 66.9% (311 실패) | **85.0%** (141) |
| 실패 집중 | KR/JA/ZH 92% | — (CJK 봇월 직격 실증) |

- `eval.yml` runs-on → `[self-hosted, ssak-mac]`. 러너 서비스 등록 완료
  (재부팅 자동시작). Mac 오프라인 시 잡 대기→180분 타임아웃 (문서화된 트레이드오프).
- 베이스라인을 Mac 러너 첫 리포트(939q/0.3617)로 **재앵커링** — 이후 회귀 감지가
  동일 환경 기준으로 작동. 래칫 상향 판정도 본 기준으로.
- 잔여: minResults 미달 141건 — 일부는 네이버 자체 per-IP 제한으로 잔존 가능.

### 게이트 재보정 (2026-08-24, 첫 939쿼리 완주 후)

첫 완주 리포트(GitHub 러너, 정상 네트워크): **939쿼리 NDCG@10 0.3394 · 통과율 83.8%**
(152건 minResults 미달 — 러너 IP 봇월로 인한 백엔드 가용성, kr/ja 풀 집중).

- **기준 변경**: 구 게이트 0.65는 600쿼리 시절(더 쉬운 풀) 기준. 풀 939 확장 +
  어버설/확장 세트 포함으로 절대값 척도 자체가 변경됨 → 게이트를 **0.32로
  재보정**(현 실측 −마진)하고 래칫 계획 명시: 검증된 개선 파동마다 +0.03 상향,
  목표 0.80은 풀 안정화 후 재판정.
- 베이스라인: 본 런 리포트를 `eval/baselines/latest.json`으로 확정 — 회귀 감지
  앵커가 최초로 939풀에 정합.
