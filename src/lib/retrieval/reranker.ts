...
function computeBM25Score(query: string, text: string): number {
  ...
  const queryTerms = query.toLowerCase().split(/[^\p{L}\p{N}]+/u).filter(t => t.length > 1)
  const docTerms = text.toLowerCase().split(/[^\p{L}\p{N}]+/u).filter(t => t.length > 1)
...
function computeURLScore(url: string): number {
  ...
  try {
    ...
    // 3. 쿼리 파라미터 수 (적을수록 좋음)
    const paramCount = parsed.searchParams.toString().split('&').filter(s => s.length > 0).length
...
function computeSemanticSimilarity(query: string, text: string): number {
  ...
  // 1. 직접 매칭
  const directMatches = (textLower.match(new RegExp(queryLower.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) ?? []).length
...
function heuristicRerank(query: string, documents: RerankDocument[], topK: number): RerankResult[] {
  ...
  const scored = documents.map((doc, i) => {
    ...
    const rerankScore = 
      ...
      0.10 * titleOverlap +        // 제목 겹침 (10%)
      0.10 * domainAuth +          // 도메인 권위 (10%)
