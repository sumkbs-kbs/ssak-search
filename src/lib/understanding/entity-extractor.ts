...
export type EntityType =
  'person' | 'organization' | 'place' | 'product' | 'technology' | 'date' | 'number' | 'url' | 'email' | 'concept'
...
const PATTERNS: Array<{
  ...
  {
    type: 'person',
    regex: /\b(?:Mr\.|Mrs\.|Ms\.|Dr\.|Prof\.|President|CEO|CTO|CFO|Founder|Chairman)\s+[A-Z][a-z]+\b/g,
    confidence: 0.6,
  },
...
function findInDictionary(text: string, dict: string[], type: EntityType): ExtractedEntity[] {
  ...
  return found.filter(
    (e, i) => !found.some((other, j) => j < i && other.startIndex <= e.startIndex && other.endIndex >= e.endIndex),
  )
}
...
export function extractEntities(query: string): ExtractionResult {
  ...
  const primaryEntity =
    entities.length > 0 ? entities.reduce((best, e) => (e.confidence > best.confidence ? e : best)) : undefined
