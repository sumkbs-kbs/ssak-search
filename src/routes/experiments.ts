...
import type { ExperimentInput, ExperimentMetric } from '../lib/experiments/ab-test'
...
// ============================================================
experimentsRoute.post('/', requireAuth as any, async (c) => {
  if (!checkBinding(c)) {
  ...
  if (input.description !== undefined && (typeof input.description !== 'string' || input.description.length > 500)) {
    return c.json<ErrorResponse>({ detail: 'description must be a string (max 500 chars)', code: 'invalid_description' }, 400)
  }
  ...
  for (const v of input.variants) {
    if (typeof v?.key !== 'string' || v.key.length === 0 || v.key.length > 20) {
      return c.json<ErrorResponse>({ detail: 'variant key must be a string (max 20 chars)', code: 'invalid_variants' }, 400)
    }
  ...
  if (input.primary_metric !== undefined && !METRICS.includes(input.primary_metric as ExperimentMetric)) {
    return c.json<ErrorResponse>({ detail: `primary_metric must be one of ${METRICS.join(', ')}`, code: 'invalid_metric' }, 400)
  }
...
// ============================================================
experimentsRoute.get('/', requireAuth as any, async (c) => {
  if (!checkBinding(c)) {
    return c.json<ErrorResponse>({ detail: 'Requires EXPERIMENT_DO binding', code: 'binding_missing' }, 501)
  }
  try {
...
// ============================================================
experimentsRoute.post('/:name/pause', requireAuth as any, async (c) => {
  if (!checkBinding(c)) {
    return c.json<ErrorResponse>({ detail: 'Requires EXPERIMENT_DO binding', code: 'binding_missing' }, 501)
  }
  try {
...
experimentsRoute.post('/:name/resume', requireAuth as any, async (c) => {
  if (!checkBinding(c)) {
    return c.json<ErrorResponse>({ detail: 'Requires EXPERIMENT_DO binding', code: 'binding_missing' }, 501)
  }
  try {
...
experimentsRoute.post('/:name/click', async (c) => {
  ...
  if (typeof body.impression_id !== 'string' || body.impression_id.length === 0 || body.impression_id.length > 64) {
    return c.json<ErrorResponse>({ detail: 'impression_id is required (string, max 64 chars)', code: 'invalid_impression_id' }, 400)
  }
...
// GET /api/experiments/:name/analyze — Bayesian analysis (auth required)
// ============================================================
experimentsRoute.get('/:name/analyze', requireAuth as any, async (c) => {
  if (!checkBinding(c)) {
    return c.json<ErrorResponse>({ detail: 'Requires EXPERIMENT_DO binding', code: 'binding_missing' }, 501)
  }
